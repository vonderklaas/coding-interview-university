...
Asymptotic analysis is a method of describing the behavior of algorithms as the input size grows,
focusing on their performance in terms of time or space complexity. As the input size increases,
the execution time of the algorithm will also increase, but it may grow linearly or
quadratically — where quadratic growth is significantly less efficient.

Common Time Complexities:

O(1) – Constant time (accessing an array element, inserting into a hash map)
O(log n) – Logarithmic time (binary search, finding height in a balanced binary search tree)
O(n) – Linear time (looping through an array, finding the max element)
O(n log n) – Linearithmic time (merge sort, heapsort)
O(n²) – Quadratic time (bubble sort, nested loops)
O(n³) – Cubic time (matrix multiplication, 3-level nested loops)
O(2ⁿ) – Exponential time (recursive Fibonacci)

- [x] [Asymptotic Notation by CS50](https://www.youtube.com/watch?v=iOq5kSKqeR4)
- [x] [Big O Notation by NeetCode](https://www.youtube.com/watch?v=BgLTDT03QtU)
...
- [x] `append("carrot")` - adds an element at the end of the array
- [x] `pop()` - remove element on index, or from end by default, return removed value
- [x] `remove("pie")` - removes index holding a value, only first occurrence
- [x] `del arr[2]` - delete an element at a specific index
- [x] `reverse()` - reverses the order of the list
- [x] `count()` - returns the number of elements with the specified value
- [x] `sort()` – sorts the list in place (optionally with a key and reverse)
- [x] `clear()` – removes all elements from the list
- [x] `copy()` – returns a shallow copy of the list
- [x] `arr[:]` – shallow copy the entire list
- [x] `arr[start:]` – slice from start to the end
- [x] `arr[:stop]` – slice from beginning to stop (exclusive)
- [x] `arr[::-1]` – reverse the list
